...
function buildKeyPatterns(keys: RedisKey[]): string[] {
  ...
  for (const { key } of keys) {
    ...
    // real prefixes (actual key segments, no substitution)
    for (let i = 1; i <= parts.length; i++) {
      patterns.add(parts.slice(0, i).join(":"));
    }
    // wildcard template prefixes
...
function RedisBrowserLayout({ children }: { children: React.ReactNode }) {
  ...
  function selectKey(key: string) {
    router.push(`/connections/redis?id=${connectionId}&db=${encodeURIComponent(activeDb)}&key=${encodeURIComponent(key)}`);
  }

  function navigate(target: "data" | "metrics") {
    if (target === "metrics") {
      router.push(`/connections/redis?id=${connectionId}&db=${encodeURIComponent(activeDb)}&view=metrics`);
    } else {
      router.push(`/connections/redis?id=${connectionId}&db=${encodeURIComponent(activeDb)}`);
    }
  }
...
export default function RedisLayout({ children }: { children: React.ReactNode }) {
  return (
    <Suspense>
      <RedisBrowserLayout>{children}</RedisBrowserLayout>
    </Suspense>
  );
}
