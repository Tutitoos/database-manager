...
function DocumentBrowserLayout({ children }: { children: React.ReactNode }) {
  ...
  function selectCollection(db: string, collection: string) {
    router.push(`/connections/document?id=${connectionId}&db=${encodeURIComponent(db)}&collection=${encodeURIComponent(collection)}`);
  }

  function navigate(target: "data" | "metrics") {
    const db = activeDb || databases[0] || "";
    if (target === "metrics") {
      router.push(`/connections/document?id=${connectionId}&db=${encodeURIComponent(db)}&view=metrics`);
    } else {
      router.push(`/connections/document?id=${connectionId}&db=${encodeURIComponent(activeDb)}&collection=${encodeURIComponent(activeCollection)}`);
    }
  }
...
export default function DocumentLayout({ children }: { children: React.ReactNode }) {
  return (
    <Suspense>
      <DocumentBrowserLayout>{children}</DocumentBrowserLayout>
    </Suspense>
  );
}
